...
def main():
    ...
    for group_number in sorted(os.listdir(folderpath)):
        for i in range(1, 51):
            ...
            ann_file = (
                folderpath + '/' + group_number + '/' + '{:0>2}'.format(i) + '.ann')
            with open(ann_file, 'r') as inp:
                text = inp.readlines()
            
            file_info.append(find_main_emotion(text))
            file_info.append(group_number + '/' + '{:0>2}'.format(i) + '.txt')

            text_file = (
                folderpath + '/' + group_number + '/' + '{:0>2}'.format(i) + '.txt')
    ...
    with open('trainset.txt', 'w') as outp:
        outp.write('\n'.join(output_list))
